...
it('makes every area necessary, keeps enough supply, and offers safe access across 100 seeds',()=>{
 for(let seed=0;seed<100;seed++) {
  ...
  for(const site of s.world.samples.filter(a=>['root-partner','phosphorus-helper','root-risk'].includes(a.id))){for(let dy=0;dy<=5;dy++)expect([5,6]).not.toContain(tileAt(s.world,site.x,site.y+dy));for(let dx=0;dx<=7;dx++)expect([5,6]).not.toContain(tileAt(s.world,site.x+dx,site.y+5));}
