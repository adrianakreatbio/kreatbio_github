...
try{
 ...
 await page.addInitScript(state=>{if(!sessionStorage.getItem('kreatbio.microload.save'))sessionStorage.setItem('kreatbio.microload.save',JSON.stringify({version:4,state}));},fixture());
 ...
 await page.addInitScript(state=>{localStorage.setItem('microload.assistReturn','true');if(!sessionStorage.getItem('kreatbio.microload.save'))sessionStorage.setItem('kreatbio.microload.save',JSON.stringify({version:4,state}));},fixture());
 await page.goto(process.env.BASE_URL||'http://localhost:5173');await page.getByRole('button',{name:'Continue culture'}).click();
 await page.keyboard.down('d');await page.waitForFunction(()=>document.querySelector('#toast').textContent.startsWith('Cargo full'));
 ...
 const read2=()=>page.evaluate(()=>{window.dispatchEvent(new Event('pagehide'));return JSON.parse(sessionStorage.getItem('kreatbio.microload.save')).state;});
