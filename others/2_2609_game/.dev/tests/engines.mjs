...
for(const [name,type] of [['webkit',webkit],['firefox',firefox]]){
 ...
 try{
  ...
  const read=()=>page.evaluate(()=>{window.dispatchEvent(new Event('pagehide'));return JSON.parse(sessionStorage.getItem('kreatbio.microload.save')).state;});const stopped=await read();assert.ok(stopped.player.y>3);await page.waitForTimeout(300);assert.deepEqual((await read()).player,stopped.player);
