...
it('by default, full cargo stays put and is delivered by returning HOME yourself',()=>{
 const s=newGame(21),sim=new Simulation(s);s.scans=[...SAMPLE_IDS];
 s.player={x:20,y:10,energy:100,health:70,cargo:Array(cargoMax(s)-1).fill('N')};
 s.world.tiles[index(20,10)]=0;s.world.tiles[index(21,10)]=3;
 sim.step(.3,1,0);
 expect(s.player.cargo).toHaveLength(cargoMax(s));
 expect(s.player.x).toBe(21);expect(s.player.y).toBe(10);
 sim.step(1/60,0,0);
 expect(sim.events.filter(e=>e.kind==='full')).toHaveLength(1);
 expect(sim.events.some(e=>e.kind==='auto-return')).toBe(false);
 sim.step(1/60,0,0);expect(sim.events.filter(e=>e.kind==='full')).toHaveLength(1);
 s.player.x=HOME.x;s.player.y=HOME.y;sim.step(1/60,0,0);
 expect(s.player.cargo).toHaveLength(0);expect(s.deposited.N).toBe(cargoMax(s)-1);expect(s.deposited.P).toBe(1);
 expect(s.bank).toBe(cargoMax(s)*10);expect(s.trips).toBe(1);expect(s.deaths).toBe(0);
});
it('assisted return delivers the final cargo item once, at every storage level, without counting a death',()=>{
 for(const level of [0,1,2,3]) {
  const s=newGame(21),sim=new Simulation(s);sim.assistReturn=true;s.scans=[...SAMPLE_IDS];s.upgrades.storage=level;s.player={x:20,y:10,energy:100,health:70,cargo:Array(cargoMax(s)-1).fill('N')};
  s.world.tiles[index(20,10)]=0;s.world.tiles[index(21,10)]=3;
  sim.step(.3,1,0);
...
it('can win via assisted delivery while preserving completed scans',()=>{
 const s=newGame(21),sim=new Simulation(s);sim.assistReturn=true;s.player.y=10;s.world.tiles[index(20,10)]=0;s.world.tiles[index(21,10)]=4;
