...
it('varies sample positions with a fixed science-bound order and safe surveyed bypasses',()=>{
 ...
 // Identities are depth-bound (Rhizobium shallow, Fusarium spores deep); positions vary.
 expect(layouts.size).toBeGreaterThan(90);expect(orders.size).toBe(1);
});
...
it('storage extends energy endurance, and every upgrade is affordable within the mission',()=>{
 ...
 expect(Object.values(PRICES).flat().reduce((a,b)=>a+b,0)).toBe(16*VALUES[0]+16*VALUES[1]+16*VALUES[2]+DEEP_K_POCKETS*VALUES[2]+(SAMPLE_IDS.length-SURVEY_IDS.length)*DISCOVERY_BONUS);
...
it('requires each area scan before any nutrient is visible or collectible',()=>{
 for(let seed=0;seed<30;seed++) {
  ...
  expect(s.world.samples[0]).toMatchObject({x:20,y:9});
  ...
  s.player.x=20;s.player.y=8;expect(sim.scan(s.world.samples[0].id)).toBe(true);
  expect(nutrientRevealed(s.world,s.scans,5)).toBe(true);
  const trio=s.world.samples.filter(a=>(SURVEY_IDS as readonly string[]).includes(a.id));
  for(const site of trio.slice(1)) expect(nutrientRevealed(s.world,s.scans,site.y)).toBe(false);
...
it('digging before a scan yields no cargo, while recovery stays locked until scanning',()=>{
 ...
 s.player.y=8;sim.scan(s.world.samples[0].id);expect(playableTile(s.world,s.scans,21,10)).toBe(2);
});
