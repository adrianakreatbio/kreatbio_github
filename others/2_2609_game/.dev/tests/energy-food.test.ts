...
it('places six reachable food blocks, including one near the start, across 100 seeds', () => {
 for(let seed=0;seed<100;seed++) {
  const s=newGame(seed), food=s.world.tiles.flatMap((t,i)=>t===FOOD_TILE?[i]:[]);
  expect(food).toHaveLength(6);expect(tileAt(s.world,19,7)).toBe(FOOD_TILE);
  for(const id of food) {
   const y=Math.floor(id/40); expect(s.world.samples.some(site=>index(site.x,site.y)===id)).toBe(false);
...
it('caps refills at the upgraded energy maximum and never awards the same food twice',()=>{
 ...
 const db=storage();save(db,s);const loaded=load(db).state!;expect(loaded.world.tiles.filter(t=>t===8)).toHaveLength(5);
 new Simulation(loaded).respawn();expect(loaded.world.tiles.filter(t=>t===8)).toHaveLength(5);
});
it('adds food to v2 saves once while preserving player, cleared tunnels, progress and completed scans',()=>{
 const s=newGame(77);s.world.tiles=s.world.tiles.map(t=>t===8?0:t);s.player.x=19;s.player.y=7;s.player.energy=50;s.bank=70;s.deposited={N:4,P:5,K:6};s.scans=['root-partner'];
 ...
 expect(migrated.world.tiles.filter(t=>t===8)).toHaveLength(6);
 migrated.world.tiles[index(21,7)]=0;save(db,migrated);expect(JSON.parse(db.getItem(SAVE_KEY)!).version).toBe(VERSION);
 expect(load(db).state!.world.tiles.filter(t=>t===8)).toHaveLength(5);
});
