...
<div class="play-layout"><div class="viewport" id="viewport" aria-label="Hold anywhere and drag to move"><canvas id="world" aria-label="Underground world. Touch and drag anywhere, or use WASD or arrow keys, to move and dig."></canvas><div id="joystick-base" hidden><div id="joystick-knob"></div></div><div class="viewport-top"><span id="colony-arrow"></span><span id="depth"></span></div><div id="survey-status"></div><div id="vital-warning" role="status" hidden></div><div id="toast" role="status"></div><div id="growth-feedback" hidden></div><div class="viewport-bottom"><span id="location"></span><span id="dig-status"></span></div><div id="navigation-reader" class="sr-only" aria-live="polite"></div><div id="overlay" class="overlay"></div></div></div>
<div class="game-controls"><p id="objective-copy" role="status"></p><button id="scan" class="primary">Scan</button></div></section>
<div class="field-notes"><span class="food-key">⚡ FOOD · +30 ENERGY</span><span><i class="dot lime"></i> N · P · K</span><span><i class="dot coral"></i> FICTIONAL HAZARDS</span><span><i class="dot blue"></i> LOW-O₂ WATER</span><span><i class="dot gold"></i> SOIL DNA SAMPLES</span><span class="field-note">Read the soil. Help life grow.</span></div><footer><span>Grown for curious lab minds by <a href="https://kreatbio.com/">KreatBio</a>.</span><span>Progress lasts only for this visit.</span></footer></main>`;
let prefsStorage: Storage;
try { prefsStorage = localStorage; } catch { prefsStorage = { getItem() { throw Error(); }, setItem() { throw Error(); } } as unknown as Storage; }
let storage: Storage;
try { storage = sessionStorage; } catch { storage = { getItem() { throw Error(); }, setItem() { throw Error(); } } as unknown as Storage; }
...
try { textNavigation=prefsStorage.getItem('microload.textNavigation')==='true';largeText=prefsStorage.getItem('microload.largeText')==='true';assistReturn=prefsStorage.getItem('microload.assistReturn')==='true'; } catch {}
...
input.bindDrag($('viewport'), $('joystick-base'), $('joystick-knob'), () => mode === 'playing');
...
function renderOverlay() {
  ...
  o.querySelectorAll<HTMLButtonElement>('button[data-action]').forEach(b => b.onclick = () => {
    ...
    switch (b.dataset.action) {
      case 'text-size': largeText=!largeText;document.body.classList.toggle('large-text',largeText);try{prefsStorage.setItem('microload.largeText',String(largeText));}catch{}renderOverlay();break;
      case 'text-navigation': textNavigation=!textNavigation;try{prefsStorage.setItem('microload.textNavigation',String(textNavigation));}catch{}renderOverlay();break;
      case 'assist-return': assistReturn=!assistReturn;sim.assistReturn=assistReturn;try{prefsStorage.setItem('microload.assistReturn',String(assistReturn));}catch{}renderOverlay();break;
...
function dragHint() {
  return `<svg class="drag-hint" viewBox="0 0 120 120" aria-hidden="true"><circle cx="60" cy="60" r="18" fill="none" stroke="#72eadc" stroke-width="2"/><circle cx="60" cy="60" r="7" fill="#d4ff70"/><g stroke="#d4ff70" stroke-width="3" fill="none" stroke-linecap="round" stroke-linejoin="round"><path d="M60 34V18M53 25l7-7 7 7"/><path d="M60 86v16M53 95l7 7 7-7"/><path d="M34 60H18M25 53l-7 7 7 7"/><path d="M86 60h16M95 53l7 7-7 7"/></g></svg><p class="drag-caption">Touch anywhere on the world and drag — up, down, left or right — to move. Let go to stop.</p>`;
}
...
function frame(now: number) {
  ...
  if (mode === 'playing') {
    ...
    while (accumulator >= 1 / 60 && mode === 'playing') {
      ...
      if(textNavigation && (sim.events.some(e=>['collect','dig','food','hurt'].includes(e.kind)) || inputMoved)){input.stopUntilRelease();accumulator=0;break;}
