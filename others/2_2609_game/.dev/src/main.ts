import { MICROBES, NUTRIENTS, NUTRIENT_INFO, PLANT_TARGET, SAMPLE_IDS, type SampleId } from './biology';
import './style.css';
import { Simulation, atHome, cargoMax, energyMax, healthMax, newGame, PRICES, TRACKS, type Track } from './simulation';
import { FOOD_TILE, HOME, layer, tileAt, playableTile, nutrientRevealed, surveyForTile, CHALLENGES } from './world';
...
<div class="game-hud" aria-label="Mission progress and vitals"><div id="plant-display"></div><div class="hud-meters"><div id="nutrient-totals"></div><div class="vital-meters">${['energy','health'].map(id => `<div class="hud-meter" id="${id}-panel"><label for="${id}">${id === 'energy' ? 'Energy' : 'Health'}</label><span id="${id}-text"></span><progress id="${id}" max="160" value="160"></progress></div>`).join('')}<div class="hud-meter"><label for="cargo">Cargo</label><span id="cargo-text"></span><progress id="cargo" max="10" value="0"></progress></div></div></div><div class="hud-actions"><span id="scan-count">DNA 0/3</span><button id="evolve" class="secondary">Upgrade · ✦ <span id="bank">0</span></button></div></div>
<div class="play-layout"><div class="viewport"><canvas id="world" aria-label="Underground world. Hold a direction on the pad, WASD or arrow keys to move and dig."></canvas><div class="viewport-top"><span id="colony-arrow"></span><span id="depth"></span></div><div id="survey-status"></div><div id="vital-warning" role="status" hidden></div><div id="toast" role="status"></div><div id="growth-feedback" hidden></div><div class="viewport-bottom"><span id="location"></span><span id="dig-status"></span></div><div id="navigation-reader" class="sr-only" aria-live="polite"></div><div id="overlay" class="overlay"></div></div></div>
<div class="game-controls"><div id="direction-pad" aria-label="Hold a direction to move"><button data-direction="up" aria-label="Move up">↑</button><button data-direction="left" aria-label="Move left">←</button><span aria-hidden="true">HOLD</span><button data-direction="right" aria-label="Move right">→</button><button data-direction="down" aria-label="Move down">↓</button></div><p id="objective-copy" role="status"></p><button id="scan" class="primary">Scan</button></div></section>
<div class="field-notes"><span class="food-key">⚡ FOOD · +30 ENERGY</span><span><i class="dot lime"></i> N · P · K</span><span><i class="dot coral"></i> FICTIONAL HAZARDS</span><span><i class="dot gold"></i> SOIL DNA SAMPLES</span><span class="field-note">Read the soil. Help life grow.</span></div><footer><span>Grown for curious lab minds by <a href="https://kreatbio.com/">KreatBio</a>.</span><span>Progress stays in this browser.</span></footer></main>`;
let storage: Storage;
...
let textNavigation=false, largeText=false;
try { textNavigation=storage.getItem('microload.textNavigation')==='true';largeText=storage.getItem('microload.largeText')==='true'; } catch {}
...
function renderOverlay() {
  ...
  let content = '';
  if (mode === 'title') content = `<img class="hero-mascot" src="${asset('base-muncher-bacterium.png')}" alt="A smiling purple bacterium"><p class="eyebrow">SMALL CELL. REAL CONNECTIONS.</p><h2>Read the soil.<br><em>Help life grow.</em></h2><p>Restore the pea plant: deliver 12 each of N, P and K, and scan 3 soil samples.</p><button class="primary" data-action="start">${hasSave ? 'Continue culture →' : 'Begin the adventure →'}</button>${hasSave ? '<button class="text-button" data-action="new">Start a new culture</button>' : ''}<small class="duration">HOLD TO MOVE · TOUCH OR KEYBOARD</small>${saved.message ? `<p class="warning">${saved.message}</p>` : ''}`;
  if (mode === 'tutorial') content = `<p class="eyebrow">YOUR FIRST SCAN</p><h2>Start beside <em>HOME.</em></h2><p>Hold ↓ for one step. Tap the glowing Scan button to reveal nutrients.</p><p>Goal: deliver 12 each of N, P and K. Scan all 3 areas.</p><p class="challenge">Optional challenge: ${CHALLENGES[sim.state.world.challenge??0]}</p><button class="primary" data-action="play">Let’s dig →</button><small class="duration">HOLD TO MOVE · RELEASE TO STOP · RULES IN NOTES</small>`;
  ...
  if (mode === 'victory') {
    ...
    content=`<p class="eyebrow">PEA PLANT RESTORED</p><h2>You helped <em>life grow.</em></h2>${plantMarkup()}<p>36 nutrients delivered. Three soil communities explored.</p><p class="challenge">${earned?'✦ Challenge complete':'Challenge to try again'} · ${CHALLENGES[challenge]}</p><div class="discovery-summary">${s.scans.map(id=>`<p><b>${MICROBES[id].role}</b><span>${MICROBES[id].short}</span></p>`).join('')}</div><div class="results"><span><b>${s.trips}</b>DELIVERIES</span><span><b>${s.deaths}</b>CARGO LOSSES</span></div><button class="primary" data-action="new">Try another challenge →</button><button class="secondary" data-action="journal">Explore Field Journal</button>`;
  }
  if(mode==='settings') content=`<p class="eyebrow">ACCESSIBILITY & SOUND</p><h2>Make it <em>comfortable.</em></h2><button class="secondary" data-action="text-size">Larger text: ${largeText?'ON':'OFF'}</button><button class="secondary" data-action="text-navigation">Text navigation: ${textNavigation?'ON':'OFF'}</button><p>Text navigation describes nearby tiles. Press an arrow once per tile; press R to hear your surroundings. It also works with direction buttons.</p><label for="volume">Sound volume</label><input id="volume" type="range" min="0" max="100" value="${Math.round(sound.volume*100)}"><button class="primary" data-action="close-settings">Back to game →</button>`;
  ...
  if (mode === 'scan-result' && scanningId) {
    const m = MICROBES[scanningId], risk = scanningId === 'root-risk';
    content = `<p class="eyebrow">SOIL RESULT / ${sim.state.scans.length} OF 3 SAMPLES COMPLETE</p><h2>${m.role}</h2><p class="scan-species"><i>${m.name}</i><br><small>Simulated DNA match · one member of this soil community</small></p><div class="scan-explanation"><h3>Role</h3><p>${m.short}</p><h3>Next</h3><p>${risk ? 'Potential pathogen detected. Use the dashed safe route; the soil survey also revealed nutrients.' : `N/P/K deposits are now unlocked. ${m.nutrient==='N'?'Follow the N outlines: nitrogen supports proteins and chlorophyll.':'Follow the P outlines: phosphorus is part of DNA and ATP.'}`}</p>${risk ? '<p class="scan-context">This DNA clue alone does not prove disease.</p>' : ''}</div><p class="map-ready">Nutrients revealed — close × to collect.</p><button class="secondary" data-action="journal">More in Field Journal</button><small class="duration">GAME PAUSED · CLOSE WITH × WHEN YOU’RE READY</small>`;
  }
  if (mode === 'journal') content = `<p class="eyebrow">KREATBIO / FIELD JOURNAL</p><h2>Your soil <em>discoveries.</em></h2><p>DNA matches reveal community clues. Nutrient tests and plant condition also matter. All matches and timings here are simulated.</p><div class="journal-list"><article><h3>Controls & survival</h3><p>Hold the direction pad, WASD or arrows to move and dig. Slide between arrows to turn; release to stop. Tap Scan or E near a gold marker. Deliver 12 each of N/P/K and scan all 3 samples, then return HOME to win.</p><p>Moving and digging use energy. HOME refills energy, repairs health and deposits cargo. Orange food restores up to 30 energy. Pink hazards and enemies damage health. At zero energy or health, you return HOME and lose carried cargo.</p><p>Full cargo returns HOME automatically. Each nutrient delivered earns 10 credits. Buy one-time upgrades at HOME; plant progress stays. Bigger storage also reduces energy use. Armor reduces hazard damage by one third. The marked safe route avoids pink shortcut hazards. Lost cargo returns to the soil; it is not delivered or credited.</p><p>N/P/K stay hidden and cannot be collected until the nearby sample is scanned. Digging before scanning preserves hidden deposits in the tunnel. After scanning, walk over them to collect. DNA identifies a microbe; the paired soil survey maps deposits or a safe route. Neither is a fertility diagnosis.</p></article><article><h3>⚡ Organic food · energy for your microbe</h3><p>Orange food restores up to 30 energy immediately and disappears. It uses no cargo slot. N/P/K count toward your plant goal instead. Many soil microbes get energy from organic material; this refill is a simplified game mechanic.</p><a href="https://www.fao.org/4/a0100e/a0100e0d.htm" target="_blank" rel="noopener noreferrer">Read the science ↗</a></article>${NUTRIENTS.map(n => `<article><h3>${n} · ${NUTRIENT_INFO[n].name}</h3><p>${NUTRIENT_INFO[n].detail}</p><a href="${NUTRIENT_INFO[n].source}" target="_blank" rel="noopener noreferrer">Read the science ↗</a></article>`).join('')}${SAMPLE_IDS.map(id => { const m = MICROBES[id]; return sim.state.scans.includes(id) ? `<article><span class="eyebrow">${m.role}</span><h3><i>${m.name}</i></h3><p>${m.detail}</p><p class="community">Community: featured DNA match + other bacteria and fungi. ${m.condition}.</p><a href="${m.source}" target="_blank" rel="noopener noreferrer">Read the science ↗</a></article>` : `<article><h3>Unsequenced soil sample</h3><p>Follow the sample compass and press E at its marker.</p></article>`; }).join('')}</div><button class="primary" data-action="close-journal">Back to ${journalReturn === 'scan-result' ? 'soil result' : journalReturn === 'victory' ? 'results' : journalReturn === 'paused' ? 'pause menu' : 'exploration'} →</button>`;
...
function updateHUD() {
  ...
  $('scan-count').textContent = `DNA ${s.scans.length}/3`;
  $('depth').textContent = `${Math.max(0,p.y-3)} tiles`;
  $('layer-label').textContent = ['ROOT SOIL','MINERAL SOIL','DEEP SOIL'][layer(p.y)];
  const dx = HOME.x-p.x, dy=HOME.y-p.y;
  $('colony-arrow').innerHTML = `<b style="display:inline-block;transform:rotate(${Math.atan2(dy,dx)*180/Math.PI}deg)">➜</b> HOME · ${Math.abs(dx)+Math.abs(dy)}`;
  const warnings = [];
  ...
  const site = sim.nearbySample(), next = s.world.samples.find(a => !s.scans.includes(a.id));
  const t = tileAt(s.world,p.x+sim.facing.x,p.y+sim.facing.y);
  $('location').textContent = next ? `SAMPLE ${s.scans.length+1}/3 · ${next.y>p.y?'↓':next.y<p.y?'↑':next.x>p.x?'→':'←'} ${Math.abs(next.x-p.x)+Math.abs(next.y-p.y)}` : 'ALL SAMPLES SCANNED';
  ...
  $('dig-status').textContent = t===6 ? 'BEDROCK' : t===FOOD_TILE ? '⚡ +30' : p.cargo.length >= cargoMax(s)-2 ? 'FULL → AUTO HOME' : '';
  const ready=s.scans.length===3 && NUTRIENTS.every(n=>s.deposited[n]+p.cargo.filter(v=>v===n).length>=12);
  ...
  const prompt=s.won?'Pea plant restored!':ready?'Plant ready! Return HOME to finish.':!s.scans.length ? site?'Tap Scan to reveal your first nutrients.':home?'Hold ↓ one step. Scan beside HOME.':'Follow SAMPLE to your first scan.':er<=.25?'Low energy: HOME or orange food.': !s.trips ? !p.cargo.length?'Collect a glowing lettered tile.':`Good! ${p.cargo.length}/${cargoMax(s)} cargo. Full → auto HOME.`:site&&!s.scans.includes(site.id)?'Tap Scan to unlock this area.':!surveyed?'New area. Find its gold sample.':!remaining&&next?'This area is harvested. Find the next sample.':er<=.25?'Low energy: HOME or orange food.':'Collect N/P/K. Full cargo → HOME.';
  if($('objective-copy').textContent!==prompt)$('objective-copy').textContent=prompt;
  const narration=`Depth ${Math.max(0,p.y-3)}. Energy ${Math.ceil(er*100)} percent. Cargo ${p.cargo.length} of ${cargoMax(s)}. ${['up','right','down','left'].map((name,i)=>{const [dx,dy]=[[0,-1],[1,0],[0,1],[-1,0]][i],t=playableTile(s.world,s.scans,p.x+dx,p.y+dy);return `${name}: ${['tunnel','soil','nitrogen','phosphorus','potassium','hazard','bedrock','tunnel','energy food'][t]}`;}).join('. ')}. ${site?'Sample here. Press E.':$('location').textContent}. ${prompt}`;
  ...
  $<HTMLButtonElement>('evolve').disabled = !home || mode !== 'playing' || s.won;
...
function frame(now: number) {
  ...
  for (const event of sim.events.splice(0)) {
    if (event.kind !== 'win' && event.kind !== 'auto-return') sound.play(event.kind === 'scan' ? 'fragment' : event.kind === 'food' ? 'collect' : event.kind); renderer.burst(event.x, event.y, event.kind === 'hurt' ? '#ff7595' : '#d4ff70');
    ...
    if (event.kind === 'deposit') {
      ...
      notify(`${milestone} ${total}/36 delivered.`,5);
